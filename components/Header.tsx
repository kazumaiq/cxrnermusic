...
export default async function Header() {
  ...
  return <header className="fixed top-0 z-50 w-full border-b border-white/10 bg-night/70 backdrop-blur-2xl">
    <Container className="flex h-[4.5rem] items-center justify-between">
      ...
      <details className="relative md:hidden"><summary className="flex h-10 w-10 cursor-pointer list-none items-center justify-center rounded-full border border-white/15 text-white/80" aria-label="Открыть меню">☰</summary><nav className="absolute right-0 top-14 w-[min(14rem,calc(100vw-2rem))] rounded-2xl border border-white/10 bg-[#130b22]/95 p-3 shadow-2xl backdrop-blur-xl">{navItems.map(item => <Link key={item.href} href={item.href} className="block rounded-xl px-4 py-3 text-sm text-white/75 transition hover:bg-white/10 hover:text-white">{item.label}</Link>)}<div className="my-2 border-t border-white/10" />{isLoggedIn ? <Link href="/dashboard" className="block rounded-xl border border-white/10 px-4 py-3 text-center text-sm text-white/80">Кабинет</Link> : <Link href="/auth/login" className="block rounded-xl border border-white/10 px-4 py-3 text-center text-sm text-white/80">Войти</Link>}<Link href="/contact" className="mt-2 block rounded-xl bg-neon px-4 py-3 text-center text-sm font-semibold text-night">Связаться</Link></nav></details>
    </Container>
  </header>;
}
